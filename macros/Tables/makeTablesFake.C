...
void makeTablesFake()
{
  ...
  TString anaCfg = getEnv("MIT_ANAJ_CFG");
  ...
  // define infolder
  TString inFileName = here + "/monoph-2013-Oct1_reduced_fake.root";
  std::cout << "inFileName " << inFileName << std::endl;
  
  //Make the tables
  //Default cut
  //==Fake rate region==
  TString thisCut = "*(metCor < 30 && nalljets < 2 && (phoIsFake == 2 || phoIsFake == 3))";
  ...
  makeTable("nphotons", thisCut, "nphotons", "nphotons", "", 
               ...
               0);
  //==High met region==
  thisCut = "*((phoIsFake == 2 || phoIsFake == 3) && metCor > 100 && nlep == 0 && phoMetDeltaPhi > 0.5 && (nalljets == 0 || (nalljets == 1 && jetMetDeltaPhi > 0.5 && jetMetDeltaPhi < 2.65 && phoJetDeltaPhi < 2.9)))";
               ...
               float* xlowVec)
{
  ...
  for (UInt_t iDatas=0; iDatas < listOfDatasets.size(); iDatas++) {
    ...
    //fill the histogram
    if ( iDatas == 0 ) treedata[iDatas] -> Draw(myVar + " >> hdata","evt_weight*kf_weight*pu_weight" + myCut,"goff");
    else treedata[iDatas] -> Draw(myVar + " >>+ hdata","evt_weight*kf_weight*pu_weight" + myCut,"goff");
  ...
  for (UInt_t iSample=0; iSample < listOfSamples.size(); iSample++) {
    ...
    if (isFirstOfSerie) tree[iSample] -> Draw(myVar + " >> " + TString(hist[theHistCounter] -> GetName()),thisScale + "evt_weight*kf_weight*pu_weight" + myCut,"goff");
    else tree[iSample] -> Draw(myVar + " >>+ " + TString(hist[theHistCounter] -> GetName()),thisScale + "evt_weight*kf_weight*pu_weight" + myCut,"goff");
