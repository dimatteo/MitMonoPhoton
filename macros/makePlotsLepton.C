...
void makePlotsLepton(bool isBlind=0, bool drawSignal=0)
{
  ...
  // define infolder
  TString inFileName = here + "/monoph-2013-Oct1_reduced_Lepton.root";
  std::cout << "inFileName " << inFileName << std::endl;
  
  //Make the histos
  //==Default cut==
  TString thisCut = "*(phoEt > 160 && nlep == 1 && phoMetDeltaPhi > 0.5 && (nalljets == 0 || (nalljets == 1 && jetMetDeltaPhi > 0.5 && jetMetDeltaPhi < 2.65 && phoJetDeltaPhi < 2.9)))";
  ...
  makeStack("bosonMass", thisCut, "bosonMass", "m_{T}(W)", "GeV", 
               listOfSignals, listOfSamples, listOfDatasets, 
               inFileName,
               isBlind, false, drawSignal, true,
               10, 0, 100,
