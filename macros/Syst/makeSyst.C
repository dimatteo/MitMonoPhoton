...
void makeSyst()
{
  ...
  // define infolder
  TString inFileName = here + "/monoph-2013-Oct1_reduced_Signal.root";
  ...
  //for (UInt_t iSample=0; iSample < 10; iSample++) {
    ...
    for ( Int_t iEntry = 0; iEntry < nEntries; iEntry++ ) {
      ...
      float nominalMet = thistree.metCor_;
      float nominalJetPt = thistree.jet2Pt_;
      float nominalLepPt = thistree.lep1Pt_;
      TVector2 nominalMetVec;
      nominalMetVec.SetMagPhi(thistree.metCor_,thistree.metCorPhi_);

      //Step2: get the nominal yield for the standard selection and PU systematics
      bool thePresel = false;
      if (thistree.phoMetDeltaPhi_ > 2 && 
      (thistree.nalljets_ == 0 || (thistree.nalljets_ == 1 && thistree.jetMetDeltaPhi_ > 0.5 && thistree.jetMetDeltaPhi_ < 2.65 && thistree.phoJetDeltaPhi_ < 2.9))) thePresel = true;
      if ( nominalPhotonEt >= 160 && nominalLepPt < 10. && nominalJetPt < 30. && nominalMet >= 140 && thePresel ) {
        //Nominal
      ...
      if ( upPhotonEt >= 160 && nominalLepPt < 10. && nominalJetPt < 30. && photonSystUpMetVec.Mod() >= 140 && thePresel  )
        v_thisYield[3] += thisScale*thistree.evt_weight_*thistree.kf_weight_*thistree.pu_weight_*photonEffSystUp;
      if ( downPhotonEt >= 160 && nominalLepPt < 10. && nominalJetPt < 30. && photonSystDownMetVec.Mod() >= 140 && thePresel )
      ...
      if (errNLeptons < 0.) {
        ...
        if ( nominalPhotonEt >= 160 && leptonSystUpLeptonVec.Mod() < 10. && nominalJetPt < 30. && leptonSystUpMetVec.Mod() >= 140 && thePresel )
          v_thisYield[5] += thisScale*thistree.evt_weight_*thistree.kf_weight_*thistree.pu_weight_*leptonEffSystUp;
        if ( nominalPhotonEt >= 160 && leptonSystDownLeptonVec.Mod() < 10. && nominalJetPt < 30. && leptonSystDownMetVec.Mod() >= 140 && thePresel )
      ...
      jetSystUpMetVec.SetMagPhi(thistree.jet2Pt_,thistree.jet2Phi_);
      jetSystDownMetVec.SetMagPhi(thistree.jet2Pt_,thistree.jet2Phi_);
      getMetSyst(nominalMetVec, jetSystUpMetVec, jetSystDownMetVec, "jets", 
                 dummy, jetSystUpJetVec, jetSystDownJetVec,
                 thistree);

      if ( nominalPhotonEt >= 160 && nominalLepPt < 10. && jetSystUpJetVec.Mod() < 30. && jetSystUpMetVec.Mod() >= 140 && thePresel )
        v_thisYield[7] += thisScale*thistree.evt_weight_*thistree.kf_weight_*thistree.pu_weight_;
      if ( nominalPhotonEt >= 160 && nominalLepPt < 10. && jetSystDownJetVec.Mod() < 30. && jetSystDownMetVec.Mod() >= 140 && thePresel )
  ...
  //case 2: consider the uncertainties on leptons
    ...
    for (int iLep = 0; iLep < myTree.nlep_; iLep++) {
      ...
      if (iLep == 0) {
        ...
        if (myTree.lep1Mass_ > 0.05) thisScaleUnc = muScaleUnc;
      }
      else if (iLep == 1) {
