...
grep -v ^# ${MIT_MONOPH_DIR}/config/${MIT_PROD_CFG}.txt > /tmp/processMonoPhoton.$$
nLine=0

for dset in `cat /tmp/processMonoPhoton.$$|grep -v ^#|tr -s ' '|cut -d' ' -f 2`
do

  nLine=$(($nLine+1))

  # find the line to this dataset and do further analysis
  line=`sed -n ${nLine}p /tmp/processMonoPhoton.$$`
